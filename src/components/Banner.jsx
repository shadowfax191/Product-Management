/* eslint-disable react/no-unescaped-entities */

const Banner = () => {
    return (
        <div>
            <div className=" bg-base-200 my-10 md:my-20 p-3 md:p-5 lg:p-0">
  <div className="hero flex flex-col lg:flex-row-reverse items-center">
    <div className="flex-1">
    <img src="https://i.ibb.co/BjrYQHT/OTTApp-Builders-1585296767860.jpg" className="  rounded-lg shadow-2xl" />
    </div>
    <div className="flex-1 space-y-4">
      <h1 className="text-5xl font-bold text-center">Welcome to the World of Entertainment</h1>
      <p className="text-center text-lg text-primary font-semibold">Cheers to the Best Hours: Happy Hour Delights!</p>
      <p className="">we create unforgettable experiences that turn ordinary moments into extraordinary memories. Join us for Happy Hour Delights and embark on a journey filled with fun, laughter, and excitement. Get ready to indulge in the best hours of your day as you explore a world of entertainment that's second to none.</p>
      <button className="btn btn-primary">Get Started</button>
    </div>
  </div>
</div>
        </div>
    );
};

export default Banner;